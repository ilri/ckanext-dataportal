...
def getDataRequest(request, database, table, format):
    ...
    if "data" in params.keys():
        try:
            ...
            abort(
                404, "You do not have access to this resource. Decode error " + str(e)
            )
        try:
            ...
            if jdata["key"] == config["ilriextensions.getdata.key"]:
                ...
                reqTime = datetime.datetime.strptime(
                    jdata["datetime"], "%Y-%m-%d %H:%M:%S.%f"
                )
                ...
                try:
                    ...
                    # For Python 2.6.X
                    tMinutes = (now - reqTime).days * 1440 + (
                        now - reqTime
                    ).seconds / 60
    ...
    def requestInfo(self, id, resource_id, data=None):
        ...
        if toolkit.request.method == "POST":
            ...
            if "submittoken" in formdata.keys():
                ...
                if processToken(
                    requestID,
                    toolkit.request.remote_addr,
                    package_dict["id"],
                    resource_id,
                    formdata["field_token"],
                    formdata["field_resourceFormat"],
                ):
                    if resource_dict["format"].lower() != "getdata":
                        ...
                        vars = {
                            ...
                            "contentType": "downloadstart",
                            "format": formdata["field_resourceFormat"],
                            "requestData": encdata,
                        }
                        return toolkit.render(
                            "ilripages/request_info.html", extra_vars=vars
                        )
                else:
                    error_summary[
                        "token"
                    ] = "Invalid token or this token does not give you confidential access to this resource"
                    vars = {
                        "package": package_dict,
                        "resource": resource_dict,
                        "contentType": "request",
            ...
            if "requesttoken" in formdata.keys():
                ...
                result, error_summary = processRequestToken(
                    uuid.uuid4(),
                    toolkit.request.remote_addr,
                    data,
                    package_dict["id"],
            ...
            if "requestdownload" in formdata.keys():
                ...
                requestID = uuid.uuid4()
                result, error_summary = processGuest(
                    requestID,
                    toolkit.request.remote_addr,
                    resource_id,
                    data,
            ...
            if "privatelogin" in formdata.keys():
                ...
                if processUser(
                    ...
                    error_summary[
                        "Invalid credentials"
                    ] = "Invalid user or password. Or your user cannot access this resource"
                    vars = {
                        "package": package_dict,
                        "resource": resource_dict,
                        "contentType": "request",
