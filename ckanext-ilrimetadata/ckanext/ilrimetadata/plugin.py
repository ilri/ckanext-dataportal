...
def getFeaturedGroups(max=1):
    ...
    try:
        ...
        rescount = dbSession.execute(
            "select resource_id,count(resource_id) as total FROM resourcestats"
        )
...
def getResourceStats(resourceID):
    ...
    try:
        res = (
            dbSession.query(resourcestatsModel)
...
def createRegionsVocab():
    return createVocabulary(
        "ILRI_vocregions", os.path.join(PATH, "sourcetxt/ilri-regions.txt")
    )
...
def createCountriesVocab():
    return createVocabulary(
        "ILRI_voccountries", os.path.join(PATH, "sourcetxt/ilri-countries.txt")
    )


def createProductsVocab():
    return createVocabulary(
        "ILRI_vocproduct", os.path.join(PATH, "sourcetxt/products.txt")
    )
...
def createSpeciesVocab():
    return createVocabulary(
        "ILRI_vocspecies", os.path.join(PATH, "sourcetxt/ilri-commodities.txt")
    )
...
def createSubjectsVocab():
    return createVocabulary(
        "ILRI_vocsubjects", os.path.join(PATH, "sourcetxt/ilri-subjects.txt")
    )
...
class IlrimetadataPlugin(plugins.SingletonPlugin, toolkit.DefaultDatasetForm):
    ...
    def dataset_facets(self, facets_dict, package_type):
        facets_dict[
            "vocab_ILRI_vocregions"
        ] = "Regions"  # We use vocab_ because ILRI_prjregions is a vocabulary
    ...
    def before_map(self, map):
        ...
        with r.SubMapper(
            map, controller="ckanext.ilrimetadata.controller:ILRIMetadataDDIController"
        ) as getMetadataDDI:
            getMetadataDDI.connect(
                "getMetadataDDI", "/dataset/{id}/metadataddi", action="getMetadataDDI"
            )

        with r.SubMapper(
            map,
            controller="ckanext.ilrimetadata.controller:ILRIMetadataRequestInfoController",
        ) as requestInfo:
            requestInfo.connect(
                "requestInfo",
                "/dataset/{id}/resource/{resource_id}/request",
                action="requestInfo",
            )

        with r.SubMapper(
            map, controller="ckanext.ilrimetadata.controller:ILRIMetadataNDAController"
        ) as confAggrement:
            confAggrement.connect(
                "displayAgreement",
                "/dataset/{id}/resource/{resource_id}/nda",
                action="displayAgreement",
            )
            confAggrement.connect(
                "displayAgreement2", "/nda", action="displayAgreement2"
            )

        with r.SubMapper(
            map, controller="ckanext.ilrimetadata.controller:ILRIMetadataNDAController"
        ) as downloadLicense:
            downloadLicense.connect(
                "downloadLicense",
                "/dataset/{id}/resource/{resource_id}/license",
                action="displayLicense",
            )

        with r.SubMapper(
            map,
            controller="ckanext.ilrimetadata.controller:ILRIMetadataGetDatabaseController",
        ) as GetDatabase:
            GetDatabase.connect(
                "GetDatabase", "/getdata/{database}.{format}", action="GetDatabase"
            )

        with r.SubMapper(
            map,
            controller="ckanext.ilrimetadata.controller:ILRIMetadataGetTableController",
        ) as GetTable:
            GetTable.connect(
                "GetTable", "/getdata/{database}/{table}.{format}", action="GetTable"
            )

        # We override the CKAN 3 API so we can protect the resource URLs
        with r.SubMapper(
            map,
            controller="ckanext.ilrimetadata.api:ILRIMetadataAPIOverride",
        ...
        with r.SubMapper(
            map,
            controller="ckanext.ilrimetadata.api:ILRIMetadataAPI2Override",
    ...
    def get_helpers(self):
        return {
            "ILRIMetadata_getResourceCount": getResourceCount,
        ...
        schema.update(
            {
                "ILRI_actyproduct": [
                    toolkit.get_validator("ignore_missing"),
                    toolkit.get_converter("convert_to_tags")("ILRI_vocproduct"),
                ]
            }
        )
    ...
    def show_package_schema(self):
        ...
        schema.update(
            {
                "ILRI_prjsubjects": [
                    toolkit.get_converter("convert_from_tags")("ILRI_vocsubjects"),
                    toolkit.get_validator("ignore_missing"),
                ]
            }
        )
        ...
        schema.update(
            {
                "ILRI_actyregions": [
                    toolkit.get_converter("convert_from_tags")("ILRI_vocregions"),
                    toolkit.get_validator("ignore_missing"),
                ]
            }
        )
        schema.update(
            {
                "ILRI_actycountries": [
                    toolkit.get_converter("convert_from_tags")("ILRI_voccountries"),
                    toolkit.get_validator("ignore_missing"),
                ]
            }
        )
        schema.update(
            {
                "ILRI_actyspecies": [
                    toolkit.get_converter("convert_from_tags")("ILRI_vocspecies"),
                    toolkit.get_validator("ignore_missing"),
                ]
            }
        )
        ...
        schema.update(
            {
                "ILRI_actyproduct": [
                    toolkit.get_converter("convert_from_tags")("ILRI_vocproduct"),
                    toolkit.get_validator("ignore_missing"),
                ]
            }
        )
