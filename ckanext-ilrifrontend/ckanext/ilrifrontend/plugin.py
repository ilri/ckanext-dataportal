...
class IlrifrontendPlugin(plugins.SingletonPlugin):
    ...
    def before_map(self, map):
        ...
        with r.SubMapper(
            map, controller="ckanext.ilrifrontend.controller:requestAccountController"
        ) as requestAccount:
            requestAccount.connect(
                "requestAccount", "/requestaccount", action="display_requestAccount"
            )

        with r.SubMapper(
            map, controller="ckanext.ilrifrontend.controller:policyController"
        ) as policy:
            policy.connect("ILRIPolicy", "/policy", action="display_policy")

        with r.SubMapper(
            map, controller="ckanext.ilrifrontend.controller:harvestController"
        ) as harvest:
