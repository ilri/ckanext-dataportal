...
from .dbfunctions import (
    ...
    setTokenToRequest,
    getTokenData,
    getDatasetsFromToken,
    getResourcesFromToken,
    addDataSetToToken,
    removeDataSetFromToken,
    addResourceToToken,
    removeResourceFromToken,
    getRequestData,
    getRequestStats,
    sendTokenByEmail,
)
...
class statisticsController(toolkit.BaseController):
    ...
    def request_stats(self):
        ...
        if toolkit.request.method == "POST":
            postdata = clean_dict(
                dict_fns.unflatten(tuplize_dict(parse_params(toolkit.request.POST)))
            )
...
class resourceAuthController(toolkit.BaseController):
            ...
            if "deleteUser" in formdata.keys():
                ...
                if deleted:
                    ...
                    if message.index("foreign key") > 0:
                        h.flash_error(
                            "This user has linked information to it and cannot be deleted. You can deactivate it though"
                        )
                    else:
        ...
        if exists == False:
            ...
            for dataset in datasets:
                try:
                    ...
                    datasetArray.append(
                        {"id": datasetInfo["id"], "title": datasetInfo["title"]}
                    )
                    try:
                        for resource in datasetInfo["resources"]:
                            resourceArray.append(
                                {
                                    "id": datasetInfo["id"],
            ...
            if toolkit.request.method == "POST":
                ...
                if "addDataset" in formdata.keys():
                    ...
                    added, message = addDataSetToUser(
                        userID, data["datasetName"], c.userobj.name
                    )
                    if added:
                ...
                if "addResource" in formdata.keys():
                    ...
                    added, message = addResourceToUser(
                        userID, data["datasetID"], data["resourceID"], c.userobj.name
                    )
                    if added:
                        ...
                        if message.index("Duplicate") > 0:
                            h.flash_error(
                                "This user already has such resource assigned"
                            )
                        else:
                            h.flash_error(message)
                ...
                if "removeResource" in formdata.keys():
                    ...
                    removed, message = removeResourceFromUser(
        ...
        if toolkit.request.method == "POST":
            ...
            if "deleteGroup" in formdata.keys():
                ...
                if deleted:
                    ...
                    if message.index("foreign key") > 0:
                        h.flash_error(
                            "This group has linked information to it and cannot be deleted."
                        )
                    else:
                        h.flash_error(message)
    ...
    def manageGroupMembers(self, groupID):
        ...
        vars = {
            "groupData": groupData,
            "error_summary": errors,
            "action_type": "manageGroupMembers",
            "action_group": "groups",
            "members": members,
            "users": getUsers(),
        }
    ...
    def manageOneGroup(self, groupID):
        ...
        if exists == False:
            ...
            if toolkit.request.method == "POST":
                ...
                if "addDataset" in formdata.keys():
                    data["datasetName"] = formdata["datasetName"]
                    added, message = addDataSetToGroup(
                        groupID, data["datasetName"], c.userobj.name
                    )
                    if added:
                        h.flash_success(_("Successfully added the dataset"))
                    else:
                        if message.index("Duplicate") > 0:
                            h.flash_error(
                                "This group already has such dataset assigned"
                            )
                        else:
                            h.flash_error(message)
                if "removeDataset" in formdata.keys():
                    data["datasetID"] = formdata["datasetID"]
                    removed, message = removeDataSetFromGroup(
                        groupID, data["datasetID"]
                    )
                    if removed:
                        h.flash_success(_("Successfully removed the dataset"))
                    else:
                ...
                if "addResource" in formdata.keys():
                    ...
                    data["datasetID"] = resource[0]
                    data["resourceID"] = resource[1]
                    added, message = addResourceToGroup(
                        groupID, data["datasetID"], data["resourceID"], c.userobj.name
                    )
                    if added:
                        h.flash_success(_("Successfully added the resource"))
                    else:
                        if message.index("Duplicate") > 0:
                            h.flash_error(
                                "This group already has such resource assigned"
                            )
                        else:
                            h.flash_error(message)
                ...
                if "removeResource" in formdata.keys():
                    ...
                    removed, message = removeResourceFromGroup(
            ...
            if "allocateToken" in formdata.keys():
                ...
                if added:
                    ...
                    if updated:
                        h.flash_success(
                            _(
                                "Successfully assigned the token. Search for token %s and edit its dataset access"
                            )
                            % tokenID
                        )
                    else:
    ...
    def manageOneToken(self, tokenID):
        ...
        if exists == False:
            ...
            if toolkit.request.method == "POST":
                ...
                if "addDataset" in formdata.keys():
                    data["datasetName"] = formdata["datasetName"]
                    added, message = addDataSetToToken(
                        tokenID, data["datasetName"], c.userobj.name
                    )
                    if added:
                        h.flash_success(_("Successfully added the dataset"))
                    else:
                        if message.index("Duplicate") > 0:
                            h.flash_error(
                                "This token already has such dataset assigned"
                            )
                        else:
                            h.flash_error(message)
                if "removeDataset" in formdata.keys():
                    data["datasetID"] = formdata["datasetID"]
                    removed, message = removeDataSetFromToken(
                        tokenID, data["datasetID"]
                    )
                    if removed:
                        h.flash_success(_("Successfully removed the dataset"))
                    else:
                ...
                if "addResource" in formdata.keys():
                    ...
                    data["datasetID"] = resource[0]
                    data["resourceID"] = resource[1]
                    added, message = addResourceToToken(
                        tokenID, data["datasetID"], data["resourceID"], c.userobj.name
                    )
                    if added:
                        h.flash_success(_("Successfully added the resource"))
                    else:
                ...
                if "removeResource" in formdata.keys():
                    ...
                    removed, message = removeResourceFromToken(
            ...
            vars = {
                "error_summary": errors,
                "action_type": "manageOneToken",
                "action_group": "tokens",
                "tokenData": tokenData,
                "datasetArray": datasetArray,
                "resourceArray": resourceArray,
                "tokenDatasets": tokenDatasets,
    ...
    def showRequestDetails(self, requestID):
        ...
        if exists == False:
            ...
            vars = {
                "error_summary": errors,
                "action_type": "showRequestDetails",
                "action_group": "tokens",
