...
def userCanAddUsers(userID):
    ...
    result = (
        dbSession.query(adminUsersModel)
        .filter(adminUsersModel.ckan_user == userID)
        .filter(adminUsersModel.can_adduser == 1)
        .first()
    )
    if result is None:
...
def userCanManageResources(userID):
    ...
    result = (
        ...
        .filter(
            or_(
                adminUsersModel.can_addresusers == 1,
                adminUsersModel.can_addresgroups == 1,
                adminUsersModel.can_addrestokens == 1,
            )
        )
        .first()
    )
...
def userResourceAccess(userID, accessID):
    ...
    if accessID == 1:
        result = (
            dbSession.query(adminUsersModel)
            .filter(adminUsersModel.ckan_user == userID)
            .filter(adminUsersModel.can_addresusers == 1)
            .first()
        )
    ...
    if accessID == 2:
        result = (
            dbSession.query(adminUsersModel)
            .filter(adminUsersModel.ckan_user == userID)
            .filter(adminUsersModel.can_addresgroups == 1)
            .first()
        )
    ...
    if accessID == 3:
        result = (
            dbSession.query(adminUsersModel)
            .filter(adminUsersModel.ckan_user == userID)
            .filter(adminUsersModel.can_addrestokens == 1)
            .first()
        )
...
def isResourceAdmin(userID):
    ...
    result = (
        dbSession.query(adminUsersModel)
        .filter(adminUsersModel.ckan_user == userID)
        .filter(adminUsersModel.is_resAdmin == 1)
        .first()
    )
    if result is None:
        closeSession(dbSession)
...
def getUserData(userID):
    ...
    if userData is not None:
        resData = {
            ...
            "org": userData.user_org,
            "active": userData.user_active,
        }
        closeSession(dbSession)
        return True, resData
    else:
...
def updateUserPassword(userID, newPassword):
    ...
    try:
        # DBSession.begin()
        dbSession.query(userModel).filter_by(user_id=userID).update(
...
def removeDataSetFromUser(userID, datasetID):
    ...
    try:
        # DBSession.begin()
        dbSession.query(userdatasetModel).filter_by(user_id=userID).filter_by(
...
def getDatasetsFromUser(userID, ckanToolkit):
    ...
    datasets = (
        ...
        .filter(userdatasetModel.user_id == userID)
        .order_by(userdatasetModel.grant_date)
        .all()
    )
    datasetDetails = []
    for dataset in datasets:
        try:
            datasetInfo = ckanToolkit.get_action("package_show")(
                {}, {"id": dataset.dataset_id}
            )
            datsetName = datasetInfo["title"]
        except:
            datsetName = ""
        datasetDetails.append(
            {
                "dataset_id": dataset.dataset_id,
                "grant_date": dataset.grant_date,
                "grant_by": dataset.grant_by,
                "dataset_name": datsetName,
            }
        )
    closeSession(dbSession)
...
def removeResourceFromUser(userID, resourceID):
    ...
    try:
        # DBSession.begin()
        dbSession.query(useresourceModel).filter_by(user_id=userID).filter_by(
...
def getResourcesFromUser(userID, ckanToolkit):
    ...
    resources = (
        ...
        .filter(useresourceModel.user_id == userID)
        .order_by(useresourceModel.grant_date)
        .all()
    )
    resourceDetails = []
    for resource in resources:
        ...
        try:
            resourceInfo = ckanToolkit.get_action("resource_show")(
        ...
        resourceDetails.append(
            {
                ...
                "grant_date": resource.grant_date,
                "grant_by": resource.grant_by,
            }
        )
    closeSession(dbSession)
...
def updateGroupData(groupID, data):
    dbSession = getSession()
    try:
        dbSession.query(authgroupModel).filter_by(group_id=groupID).update(
...
def getGroupData(groupID):
    dbSession = getSession()
    groupData = (
        dbSession.query(authgroupModel)
        .filter(authgroupModel.group_id == groupID)
        .first()
    )
...
def getGroupMembers(groupID):
    ...
    sql = (
        "SELECT user.user_id,user.user_name,usergroup.join_date,usergroup.joined_by "
        "FROM user,usergroup "
        "WHERE usergroup.user_id = user.user_id AND usergroup.group_id = '"
        + groupID
        + "' ORDER BY usergroup.join_date"
    )
...
def removeUserFromGroup(groupID, userId):
    dbSession = getSession()
    try:
        dbSession.query(usergroupModel).filter_by(group_id=groupID).filter_by(
            user_id=userId
        ).delete()
        closeSession(dbSession)
        return True, ""
...
def removeDataSetFromGroup(groupID, datasetID):
    dbSession = getSession()
    try:
        dbSession.query(groupdatasetModel).filter_by(group_id=groupID).filter_by(
            dataset_id=datasetID
        ).delete()
...
def removeResourceFromGroup(groupID, resourceID):
    ...
    dbSession = getSession()
    try:
        dbSession.query(groupresourceModel).filter_by(group_id=groupID).filter_by(
            resource_id=resourceID
        ).delete()
...
def getDatasetsFromGroup(groupID, ckanToolkit):
    ...
    datasets = (
        ...
        .filter(groupdatasetModel.group_id == groupID)
        .order_by(groupdatasetModel.grant_date)
        .all()
    )
    datasetDetails = []
    for dataset in datasets:
...
def getResourcesFromGroup(groupID, ckanToolkit):
    ...
    resources = (
        ...
        .filter(groupresourceModel.group_id == groupID)
        .order_by(groupresourceModel.grant_date)
        .all()
    )
    resourceDetails = []
    for resource in resources:
...
def getTokenRequests(ckanToolkit):
    dbSession = getSession()
    requests = (
        dbSession.query(tokenrequestModel)
        .order_by(tokenrequestModel.request_date)
        .all()
    )
    requestDetails = []
        ...
        requestDetails.append(
            {
                ...
                "user_org": request.user_org,
                "user_country": request.user_country,
                "token_given": request.token_given,
            }
        )
    closeSession(dbSession)
...
def deleteToken(tokenID):
    dbSession = getSession()
    try:
        dbSession.query(tokenrequestModel).filter_by(token_given=tokenID).update(
            {"token_given": None}
        )
    ...
    try:
        ...
        try:
            dbSession.query(tokenModel).filter_by(token_id=tokenID).update(
                {"token_active": 0}
            )
...
def sendTokenRequestMail(body, targetName, targetEmail):
    ...
    body = add_msg_niceties(
        targetName, body, "ILRI Datasets Portal", "https://data.ilri.org/portal/"
    )
    msg = MIMEText(body.encode("utf-8"), "plain", "utf-8")
    ssubject = (
        "Your application for confidential information at https://data.ilri.org/portal/"
    )
...
def sendTokenByEmail(tokenID):
    ...
    try:
        tokenInfo = (
            dbSession.query(tokenrequestModel).filter_by(token_given=tokenID).first()
        )
        if tokenInfo is not None:
            ...
            message = (
                message + "Use the token: " + str(tokenID) + " to access the data\n\n"
            )
...
def setTokenToRequest(requestID, tokenID):
    dbSession = getSession()
    try:
        dbSession.query(tokenrequestModel).filter_by(request_id=requestID).update(
            {"token_given": tokenID}
        )
...
def getTokenData(tokenID):
    ...
    if tokenData is not None:
        resData = {
            ...
            "token_givendate": tokenData.token_givendate,
            "token_givenby": tokenData.token_givenby,
        }
        closeSession(dbSession)
        return True, resData
    else:
...
def getDatasetsFromToken(tokenID, ckanToolkit):
    ...
    datasets = (
        ...
        .filter_by(token_id=tokenID)
        .order_by(datasetokenModel.grant_date)
        .all()
    )
    datasetDetails = []
    for dataset in datasets:
...
def getResourcesFromToken(tokenID, ckanToolkit):
    ...
    resources = (
        ...
        .filter_by(token_id=tokenID)
        .order_by(resourcetokenModel.grant_date)
        .all()
    )
    resourceDetails = []
    for resource in resources:
...
def removeDataSetFromToken(tokenID, datasetID):
    dbSession = getSession()
    try:
        dbSession.query(datasetokenModel).filter_by(token_id=tokenID).filter_by(
            dataset_id=datasetID
        ).delete()
...
def removeResourceFromToken(tokenID, resourceID):
    ...
    dbSession = getSession()
    try:
        dbSession.query(resourcetokenModel).filter_by(token_id=tokenID).filter_by(
            resource_id=resourceID
        ).delete()
...
def getRequestData(requestID, ckanToolkit):
    dbSession = getSession()
    requestData = (
        dbSession.query(tokenrequestModel).filter_by(request_id=requestID).first()
    )
    if requestData is not None:
        ...
        resData = {
            ...
            "user_org": requestData.user_org,
            "user_orgtype": requestData.user_orgtype,
            "user_country": requestData.user_country,
            "user_datausage": requestData.user_datausage,
            "user_otherdata": requestData.user_otherdata,
            "user_hearfrom": requestData.user_hearfrom,
            "token_given": requestData.token_given,
        }
        closeSession(dbSession)
        return True, resData
    else:
...
def getRequestStats(
    draw, fields, start, length, orderIndex, orderDirection, searchValue
):
        ...
        for record in records:
            ...
            for field in fields:
                try:
                    ...
                    aRecord[
                        field
                    ] = "AJAX Data error. Report this error to support_for_cabi@qlands.com"
            data.append(aRecord)
        ...
        result.append(
            {
                ...
                "request_orgtype": data.request_orgtype,
                "request_country": data.request_country,
                "request_datausage": data.request_datausage,
