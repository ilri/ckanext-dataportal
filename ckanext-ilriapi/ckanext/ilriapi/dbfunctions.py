...
def getConfidentialFields(dataset):
    ...
    result = []
    try:
        cnfFields = dbSession.execute("select dict_tblinfo.tbl_cod,dict_tblinfo.tbl_des,dict_clminfo.clm_cod,dict_clminfo.clm_des "
        ...
        for field in cnfFields:
            result.append({'tblname':field[0],'tbldesc':field[1],'clmname':field[2],'clmdesc':field[3]})
